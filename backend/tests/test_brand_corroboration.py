...
def _gym_with_status(
    name: str, status: MatchStatus, lat: float = 35.920, lon: float = -86.862
) -> Gym:
    gym = Gym(
        name=name,
...
def test_close_brand_result_upgrades_tomtom_only_to_corroborated():
    ...
    assert (
        gym.source_provenance["match_status"] == MatchStatus.TOMTOM_CORROBORATED.value
    )
...
def test_distant_brand_result_leaves_tomtom_only_unchanged():
    gym = _gym_with_status(
        "CrossFit Franklin", MatchStatus.TOMTOM_ONLY, lat=35.920, lon=-86.862
    )
    ...
    gym.tags.update(
        {
            ...
            "brand": "CrossFit",
            "brand:wikidata": "Q123",
            "operator": "CrossFit HQ",
            "operator:wikidata": "Q456",
            "contact:instagram": "@cf",
            "contact:facebook": "cf",
        }
    )
    brand = _brand_result("CrossFit Franklin")
...
def test_brand_specific_query_triggers_corroboration(
    ...
    monkeypatch.setattr(
        "api.routes_v2.TomTomClient.geocode", lambda *a, **kw: [_FRANKLIN_ORIGIN]
    )
    monkeypatch.setattr(
        "api.routes_v2.TomTomClient.search_gyms", lambda *a, **kw: [_FRANKLIN_GYM]
    )
    monkeypatch.setattr(
        "api.routes_v2.TomTomClient.search", lambda *a, **kw: [_BRAND_RESULT]
    )
    ...
    assert (
        result["source_provenance"]["match_status"]
        == MatchStatus.TOMTOM_CORROBORATED.value
    )
...
def test_broad_query_does_not_trigger_brand_search(client, override_auth, monkeypatch):
    ...
    assert resp.status_code == 200
    assert brand_search_called == [], (
        "brand search must not be called for broad query 'gym'"
    )
    result = resp.json()["results"][0]
...
def test_osm_confirmed_not_overwritten_by_brand_search(
    client, override_auth, monkeypatch
):
    osm_elements = [
        {
    ...
    assert (
        result["source_provenance"]["match_status"] == MatchStatus.OSM_CONFIRMED.value
    )
